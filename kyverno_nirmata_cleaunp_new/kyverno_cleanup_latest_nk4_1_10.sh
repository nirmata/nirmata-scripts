...
if [ $# -lt 2 ]; then
    ...
    echo ""
    echo "Eg: $0 /home/user/.kube/config nirmata-system niramta-kyverno-operator kyverno"
    echo "Warning"
else
    kubeconfig=$1
    shift 1
    ...
    echo "==================================================="
        ...
        # Function to retry Helm uninstall
        retry_uninstall() {
            local chart=$1
            for attempt in {1..2}; do
                helm uninstall $chart -n $chart && break
                echo "Retrying $chart chart uninstall (attempt $attempt)..."
                sleep 5
            done
        }
        ...
        for attempt in {1..2}; do
            helm uninstall kyverno -n kyverno && break
            echo "Retrying kyverno chart uninstall (attempt $attempt)..."
            sleep 5
        done

        # Uninstall nirmata-kyverno-operator chart
        for chart in nirmata-kyverno-operator nirmata-system enterprise-kyverno-operator; do
            echo "Uninstalling $chart chart..."
            
            # Check if release exists before attempting uninstall
            if release_exists $chart $chart; then
                retry_uninstall $chart
            else
                echo "Release $chart not found. Skipping uninstallation."
            fi
        done
    ...
    echo "==================================================="
        ...
        # echo "$crds" | xargs -I {} kubectl --kubeconfig="$kubeconfig" patch {} -p '{"metadata":{"finalizers":[]}}' --type=merge
        echo "$crds" | xargs -I {} kubectl --kubeconfig="$kubeconfig" patch {} -p '{"metadata":{"finalizers":null}}' --type=merge
        retries=0
        while [[ -n $crds && $retries -lt 3 ]]; do
    ...
    echo "Deleting CustomResourceDefinitions"
    crds=$(kubectl --kubeconfig="$kubeconfig" get customresourcedefinition -o name | grep -i "security.nirmata.io")
    for crd in $crds; do
        kubectl --kubeconfig="$kubeconfig" patch $crd -p '{"metadata":{"finalizers":[]}}' --type=merge
    done

    retries=0
    while [[ -n $crds && $retries -lt 2 ]]; do
        echo "$crds" | xargs kubectl --kubeconfig="$kubeconfig" delete --force --grace-period=0
        sleep 5
        crds=$(kubectl --kubeconfig="$kubeconfig" get customresourcedefinition -o name | grep -i  "security.nirmata.io")
        ((retries++))
    ...
    echo "Remaining CustomResourceDefinitions:"
    kubectl --kubeconfig="$kubeconfig" get customresourcedefinition -o name | grep -i  "security.nirmata.io" || echo "No resources found."
    ...
    echo "==================================================="
        ...
        # Uninstall kyverno chart if previously not deleted 
        echo "Uninstalling kyverno chart..."
        helm uninstall kyverno -n kyverno --no-hooks

        # Uninstall nirmata-kyverno-operator chart if previously not deleted 
