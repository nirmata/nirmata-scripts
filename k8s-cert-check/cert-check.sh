...
if [ "${DAYS_LEFT}" -le "${WARNDAYS}" ]; then
   #     echo "The cert will expire soon!"
        echo '"Hi, k8s certs is going to expire soon on zephyr production."' | mail -r "Zephyr-prod" -s "k8s certs will be expire soon : Zephyr Prod" "anudeep.nalla@nirmata.com" # Action if true
else
