...
function Set-Proxy {
    ...
    Write-Output "Setting up $webproxy as proxy."
    <#[system.net.webrequest]::DefaultWebProxy = new-object system.net.webproxy($webproxy)
    [system.net.webrequest]::DefaultWebProxy.credentials = [System.Net.CredentialCache]::DefaultNetworkCredentials
    [system.net.webrequest]::DefaultWebProxy.BypassProxyOnLocal = $true
    #>
    #Write-Output "Setting up $webproxy as proxy."
    [Environment]::SetEnvironmentVariable("HTTPS_PROXY", $webproxy, [EnvironmentVariableTarget]::Machine)
...
function InstallPauseImage() 
{
    ...
    if (!$infraPodImage)
    {
        ...
        Invoke-WebRequest -Uri "https://github.com/$Global:GithubSDNRepository/raw/$Global:GithubSDNBranch/Kubernetes/windows/Dockerfile" -OutFile  $Global:BaseDir\Dockerfile -Proxy $Global:webproxy
...
function InstallKubernetesBinaries()
{
    ...
    Param(
        ...
        DownloadAndExtractTarGz -url $Url -dstPath $DestinationPath -Webproxy $webproxy       
}

function DownloadAndExtractTarGz($url, $dstPath, $webproxy = "http://nibr-proxy.global.nibr.novartis.net:2011")
{
    ...
    Invoke-WebRequest $url -o $tmpTarGz.FullName -Proxy $webproxy
...
function Install-7Zip()
{
    ...
    if (-not (Get-Command Expand-7Zip -ErrorAction Ignore)) {
        Install-Package -Scope CurrentUser -Force 7Zip4PowerShell -Verbose -proxy $Global:webproxy
        if(-not $?) {
...
function DownloadFlannelBinaries()
{
    ...
    Invoke-WebRequest -Uri "https://github.com/coreos/flannel/releases/download/v${Release}/flanneld.exe" -OutFile $DestinationPath\flanneld.exe -Proxy $webProxy
}

function DownloadCniBinaries()
{
    param(
        [Parameter(Mandatory = $false, Position = 0)] [string] $NetworkMode,
        [parameter(Mandatory = $false)] [string] $CniPath ,
    ...
    Invoke-WebRequest -Uri "https://github.com/$Global:GithubSDNRepository/raw/$Global:GithubSDNBranch/Kubernetes/flannel/$NetworkMode/cni/config/cni.conf" -OutFile $CniPath\config\cni.conf -Proxy $webProxy
}
...
function InstallFlannelD()
{
    ...
    [Parameter(Mandatory = $false)][string] $webproxy = "http://nibr-proxy.global.nibr.novartis.net:2011"
    )
    ...
    Invoke-WebRequest -Uri  "https://github.com/$Global:GithubSDNRepository/raw/$Global:GithubSDNBranch/Kubernetes/flannel/$Global:NetworkMode/net-conf.json" -OutFile $(GetFlannelNetConf) -Proxy $webproxy
...
function GetSourceVip($NetworkName)
{
    ...
    $ipamConfig = @"
        {"cniVersion": "0.3.0", "name": "vxlan0", "ipam":{"type":"host-local","ranges":[[{"subnet":"$subnet"}]],"dataDir":"/var/lib/cni/networks"}}
"@
    ...
    return $sourceVipAddress.Split("/")[0]
}
...
# Install Flannel and CNI Binaries
DownloadFlannelBinaries -DestinationPath $Global:BaseDir -WebProxy $webproxy
DownloadCniBinaries -NetworkMode $Global:NetworkMode -CniPath (GetCniPath) -WebProxy $webproxy

# Install FlannelD 
InstallFlannelD -Destination $Global:BaseDir -InterfaceIpAddress $Global:ManagementIp

# Update CNI Congiurations
