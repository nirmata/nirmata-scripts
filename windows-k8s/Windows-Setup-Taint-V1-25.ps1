Param(
    ...
    [parameter(Mandatory = $false)] $InterfaceName="Ethernet 3",
    [parameter(Mandatory = $false)] $Release = "1.25.5",
...
function IsContainerdInstalledAndRunning() 
{
...
function DownloadCniBinaries($NetworkMode, $CniPath)
{
    ...
    DownloadAndExtractTarGz "https://github.com/containernetworking/plugins/releases/download/v1.5.1/cni-plugins-windows-amd64-v1.5.1.tgz" "$CniPath"
...
function InstallKubelet()
{
    ...
    (
        ...
        [parameter(Mandatory=$false)] [string] $CniDir,
        [parameter(Mandatory=$false)] [string] $CniConf,
        ...
        [parameter(Mandatory = $true)] [switch] $IsContainerd = $true
    )
...
function GetKubeletArguments()
{
    ...
    (
        ...
        [parameter(Mandatory=$false)] [string] $CniConf,
        [parameter(Mandatory=$false)] [string] $KubeDnsServiceIp,
        [parameter(Mandatory=$false)] [string] $EnforceNodeAllocatable,
        [parameter(Mandatory=$false)] [string] $CgroupsPerQOS,
        [parameter(Mandatory=$true)] [string] $NodeIp,
        [parameter(Mandatory = $false)] $KubeletFeatureGates = "",
        [parameter(Mandatory = $true)] [switch] $IsContainerd = $true
    )
...
-NodeIp $Global:ManagementIp -IsContainerd $IsContainerd 
StartKubelet
